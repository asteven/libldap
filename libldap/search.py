...
class LdapSearch(object):
    ...
    def connection(self):
        if self._connection is None:
            try:
                ...
                self._connection.simple_bind_s(self.bind_dn, self.bind_pw)
    ...
    def search(self, scope=ldap.SCOPE_SUBTREE, filter='(objectclass=*)', attributes=None):
        with self as l:
            try:
                return l.search_s(self.basedn, scope, filter, attributes)
            except ldap.LDAPError as error:
    ...
    def async(self, scope=ldap.SCOPE_SUBTREE, filter='(objectclass=*)', attributes=None, timeout=0):
        with self as l:
            try:
                result_id = l.search(self.basedn, scope, filter, attributes)
